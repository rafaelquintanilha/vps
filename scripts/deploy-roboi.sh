...
require_non_empty_legacy_value() {
  local key="$1"
  local value="${!key:-}"

  if [ -z "$value" ]; then
    log "ERROR: /opt/apps/.env must define a non-empty ${key} for the first Roboi instance migration."
    exit 1
  fi
}
...
start_legacy_containers() {
  ...
  for name in "${names[@]}"; do
    if docker ps -a --format "{{.Names}}" | grep -Fxq "$name"; then
      log "Starting legacy container $name after failed migration..."
      docker start "$name" >/dev/null || true
    fi
  done
}

rollback_legacy_containers_if_needed() {
  if [ "$LEGACY_CONTAINERS_STOPPED" = "1" ]; then
    log "Deployment failed after stopping legacy Roboi containers; restarting legacy containers."
    start_legacy_containers
  fi
}
...
ensure_legacy_instance_if_needed() {
  ...
  local legacy_hosts="${ROBOI_HOSTS:-roboi.rafaelquintanilha.com,inteligencia.pontasales.com.br,roboi.69.62.89.22.nip.io}"
...
reload_caddy() {
  ...
  local caddy_env_args=()
  if [ -f "${COMPOSE_DIR}/.env" ]; then
    caddy_env_args=(--env-file "${COMPOSE_DIR}/.env")
  fi
...
trap rollback_legacy_containers_if_needed ERR
main "$@"
trap - ERR
